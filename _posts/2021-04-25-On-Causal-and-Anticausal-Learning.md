...
![fig1](/assets/img/posts/2021-04-25/fig1.png)
...
\begin{align*}E=\varphi(C,N_E)=\phi(C)+N_E\end{align*}
    $$
ANM은 residual noise 변수가 cause와 독립이도록 하면서 cause에 effect를 regressiong함으로써 fitting 시킨다.
    
이 논문에서 ANM은 중요하다: 1) causal discovery를 할 것이다. 2) ANM을 일반화해서 $\phi$를 share하는 형태의 모델을 구축할 것이다. 이를 이용해 causal direction의 영향성을 볼 것이다.
...
## 2. C → E : Predicting Effect from

![fig2](/assets/img/posts/2021-04-25/fig2.png)
...
#### 1. Additional information about the input
...
#### 2. Additional information about the output
...
#### 3. Additional information about the input and the output
...
## 3. E → C : Predicting Cause from Effect
