...
1. Causal direction에서,
   ...
   3. Transfer learning ($$\phi$$는 바뀌지 않고 노이즈는 바뀌는 경우): $$N_X\perp\!\!\!\perp N_Y|\text{condition}$$인 ANM에서는 가능합니다.
...
$$X$$로부터 $$Y$$를 예측할 때, $$X$$가 effect이고 $$Y$$가 cause인 경우(anticausal direction을 가질 때)를 ***anticausal prediction\*** 이라고 합니다. 이 경우 $P(X|Y)$가 causal 메커니즘이겠죠. 이는 독립성 가정에 의해 $$P(Y)$$와 독립입니다. 반면에, $P(Y|X)$는 $P(Y)$의 변화에 민감할텐데, 이를 추정하는 것은 
$$
P(X|Y)
$$
를 모델링하고나서 베이즈룰을 이용해 
...
![fig3](/assets/img/posts/2021-04-25/fig3.png)
